  ...
  const handleCnuiConfirm = useCallback(
    async (
      ...
      try {
        ...
        if (result.success) {
          // 操作成功消息映射
          const ACTION_MESSAGES: Record<string, (d: Record<string, unknown>) => string> = {
            createHabit: (d) => {
          ...
          const content = ACTION_MESSAGES[action]?.(result as unknown as Record<string, unknown>) ?? '操作成功！'
          const msg: ChatMessage = {
            role: "assistant",
            content,
            timestamp: new Date().toISOString(),

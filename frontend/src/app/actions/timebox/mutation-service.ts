 ...
 * domainId / repos（timebox+itinerary）/ 事件名 TimeboxFieldUpdated。
 *
 * [026] 决议 A：拆双 service。createTimeboxMutationService 发 TimeboxFieldUpdated；
 * createItineraryMutationService 发 ItineraryFieldUpdated。两 service 共享同一对 repo
...
/**
 * 组装 Itinerary 域业务事实写入口服务实例（[026] D2 reversal 决议 A）。
 * 共享 timebox+itinerary 两个 repo 实例，但 fieldUpdatedEventType 独立为 ItineraryFieldUpdated。
 * 每次调用产生独立实例（独立 eventRepo/eventBus），保证事务隔离。
 * @returns 业务事实写入口服务（fieldUpdatedEventType = ItineraryFieldUpdated）
 */
export function createItineraryMutationService(): DomainMutationService {
  ...
  return createDomainMutationServiceFactory({
    domainId: 'timebox',
    repos,
    fieldUpdatedEventType: 'ItineraryFieldUpdated',
    repoLabel: 'Itinerary',
  })
}

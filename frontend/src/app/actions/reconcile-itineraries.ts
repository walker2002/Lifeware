 ...
 *
 * [026] T8 架构决议：helper 走 `createItineraryMutationService().execute({state})`
 *   而**不**经 `submitDynamicIntent`。原因：
 *   - submitDynamicIntent 走 submitDynamicIntent→Orchestrator→RuleEngine，
 *     RuleEngine 对 `markInProgressItinerary`/`markExpiredItinerary`（系统 reconcile
 ...
 *   - T7 server actions（`markInProgressItinerary`/`markExpiredItinerary`）仍保留
...
import { createItineraryMutationService } from './timebox/mutation-service'
import type { USOM_ID } from '@/usom/types/primitives'

export async function reconcileAndAdvanceItineraries(
  userId: USOM_ID,
      ...
      const res = await service.execute(
        {
          ...
          targetId: act.itineraryId as USOM_ID,
          steps: [{ kind: 'state', action: smAction, targetId: act.itineraryId as USOM_ID }],
        },
        userId,
      )
      if (res.success) {

 ...
 * @brief [024] T12 + [023.12] T6 OKRDirectory 二级树（周期-目标）测试
 *
 * 覆盖：
 *  1. 按周期分组渲染目标（周期名 + 目标标题）
 *  2. 目标 ⋯ 菜单含「删除目标」（[022.01] Phase 3 简化为仅删除）
 *  3. 空周期显示且 ⋯ 菜单含 添加目标/删除周期
 *  4. 周期折叠逻辑（[T6] hasActive 由 in_progress→approved）
 *
...
const cycles = [{ id: 'c1', name: '2026 Q3', status: 'approved', period: { start: '2026-07-01', end: '2026-09-30' } }] as unknown as Cycle[]
...
describe('[024] OKRDirectory 二级树', () => {
  ...
  it('空周期显示且 ⋯ 含 添加目标/删除周期', async () => {
    const user = userEvent.setup()
    render(<OKRDirectory cycles={[...cycles, { id: 'c2', name: '2026 Q4', status: 'approved', period: { start: '2026-10-01', end: '2026-12-31' } } as unknown as Cycle]} objectives={objectives} statusFilter="all" onStatusFilterChange={() => {}} onSelect={() => {}} selectedId={null} onAddObjectiveToCycle={vi.fn()} onDeleteCycle={vi.fn()} />)
...
/**
 * [024.1] T1 + [023.12] T6：周期折叠
 *  - 默认：含 active 目标的周期展开；其他收起
 *  - [T6] hasActive 判断：cycle.status === 'approved'（原 in_progress）
...
describe('[024.1] OKRDirectory 周期折叠', () => {
  ...
  // [023.12] T6：in_progress→approved
  const c1 = { id: 'c1', name: '含进行中周期', status: 'approved', period: { start: '2026-07-01', end: '2026-09-30' } } as unknown as Cycle
...
/**
 * [022.01] Task 4 + [023.12] T6: 顶部筛选 tabs 改为 Cycle 状态
 *  - 筛选作用于 objective 所属 cycle 的状态，而非 objective 自身状态
 *  - statusFilter="all" → 显示所有 objective
 *  - statusFilter="approved" → 只显示 cycle.status==="approved" 的 objective
 *    （[T6] 原 in_progress）
 */
describe('[022.01] OKRDirectory filterObjectivesByCycleStatus 按 cycle 状态筛选', () => {
  ...
  //  - approved cycle 下挂一个 active objective + 一个 paused objective
  //  - draft cycle 下挂一个 draft objective
  const objs = [
    { id: 'o1', title: 'Active 目标 (approved cycle)', cycleId: 'c1' },
    { id: 'o2', title: 'Paused 目标 (approved cycle)', cycleId: 'c1' },
    { id: 'o3', title: 'Draft 目标 (draft cycle)', cycleId: 'c2' },
  ] as unknown as Objective[]
  it('statusFilter="approved" → 只显示对应 cycle 下的 objectives（无论 objective 自身 status）', () => {
    const result = filterObjectivesByCycleStatus(objs, [cApproved, cDraft], 'approved')
  ...
  it('statusFilter="all" → 显示所有 objectives（所有 cycle 下挂的都包含）', () => {
    const result = filterObjectivesByCycleStatus(objs, [cApproved, cDraft], 'all')

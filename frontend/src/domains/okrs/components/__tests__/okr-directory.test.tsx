...
const cycles = [{ id: 'c1', name: '2026 Q3', period: { start: '2026-07-01', end: '2026-09-30' } }] as Cycle[]
const objectives = [{ id: 'o1', title: '提升质量', cycleId: 'c1', status: 'active', objectiveNumber: 'O1' }] as Objective[]
...
describe('[024] OKRDirectory 二级树', () => {
  ...
  it('空周期显示且 ⋯ 含 添加目标/删除周期', async () => {
    const user = userEvent.setup()
    render(<OKRDirectory cycles={[...cycles, { id: 'c2', name: '2026 Q4', period: { start: '2026-10-01', end: '2026-12-31' } } as Cycle]} objectives={objectives} statusFilter="all" onStatusFilterChange={() => {}} onSelect={() => {}} selectedId={null} onAddObjectiveToCycle={vi.fn()} onDeleteCycle={vi.fn()} />)
...
describe('[024.1] OKRDirectory 周期折叠', () => {
  const activeObj = { id: 'o1', title: '进行中目标', cycleId: 'c1', status: 'active', objectiveNumber: 'O1' } as Objective
  const completedObj = { id: 'o2', title: '已完成目标', cycleId: 'c2', status: 'completed', objectiveNumber: 'O2' } as Objective
  const c1 = { id: 'c1', name: '含进行中周期', period: { start: '2026-07-01', end: '2026-09-30' } } as Cycle
  const c2 = { id: 'c2', name: '仅已完成周期', period: { start: '2026-04-01', end: '2026-06-30' } } as Cycle

...
describe('habits submit 聚合 habit_action_fields_valid（逐字对标 golden）', () => {
  ...
  it('createTemplate name 空 → Rejected「name 必填」', async () => {
    const r = await habitRuleRegistry.submit.habit_action_fields_valid.check(intent('createTemplate', { name: '', applicableDays: [1, 2, 3, 4, 5] }), serverCtx)
    expect(r.kind === 'Rejected' && r.errors).toEqual(['name 必填'])
  })
  it('addHabitToTemplate timeOverride 非法 → Rejected 含 timeOverride', async () => {
    const r = await habitRuleRegistry.submit.habit_action_fields_valid.check(intent('addHabitToTemplate', { templateId: 't1', habitId: 'h1', timeOverride: 'bad' }), serverCtx)

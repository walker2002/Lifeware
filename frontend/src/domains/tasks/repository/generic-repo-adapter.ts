  ...
  return {
    task: {
      async findById(id, userId) {
        return repos.taskRepo.findById(id, userId)
      },
      async save(obj, userId) {
        await repos.taskRepo.save(obj, userId)
      },
      async create(fields, userId) {
        return repos.taskRepo.create(fields, userId)
      },
      async updateStatus(id, toStatus, userId) {
        return repos.taskRepo.updateStatus(id, toStatus, userId)
      },
    },
    thread: {
      async findById(id, userId) {
        return repos.threadRepo.findById(id, userId)
      },
      async save(obj, userId) {
        await repos.threadRepo.save(obj, userId)
      },
      async create(fields, userId) {
        return repos.threadRepo.create(fields, userId)
      },
      async updateStatus(id, toStatus, userId) {
        return repos.threadRepo.updateStatus(id, toStatus, userId)
      },
    },
  }
}

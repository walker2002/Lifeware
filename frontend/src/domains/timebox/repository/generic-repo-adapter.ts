 ...
 *
 * 将 ITimeboxRepository / IItineraryRepository 适配为通用 GenericRepo 接口，
 * 使 Timebox 域（含 [026] 行程）可使用通用状态机（GenericStateMachine）处理所有状态转换。
 *
 * [026] 决议 A：adapter 接受 timeboxRepo + itineraryRepo，map 加 itinerary 键。
 ...
 * @property itineraryRepo - 行程仓储实例（[026] D2 reversal）
 */
 ...
 * @param repos - 包含 timeboxRepo 与 itineraryRepo 的对象
...
export function createTimeboxGenericRepo(repos: TimeboxRepoPair): Record<string, GenericRepo> {
  return {
    ...
    // [026] D2 reversal: 行程独立 GenericRepo 键，updateStatus 完整支持 5 态（4 active + completed 占位）
    itinerary: {
      async findById(id, userId, tx) {
        return repos.itineraryRepo.findById(id, userId, tx)
      },
      async save(obj, userId, tx) {

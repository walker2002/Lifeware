/**
 * @file reconcile-itinerary-tz.test.ts
 * @brief reconcileItineraryStatuses 跨 TZ 边界单元测试（[026] T20 codex #5）
 *
 * 守护 [026] OQ-6 风险：「同一日历日歧义」—— reconcile-itinerary.ts 用
 ...
 * 都能正确归类 未来/当日/过日。
 *
 * 既有 reconcile-itinerary.test.ts 已用「本地正午无 TZ 后缀」fixture 规避 TZ 差异
...
/**
 * helper: 构造一个行程 fixture（scheduled + 给定 startTime ISO）
...
describe('reconcileItineraryStatuses — 跨 TZ 边界（[026] T20 codex #5）', () => {
  it('TZ=Asia/Shanghai (UTC+8)：行程日=2026-07-15 本地，当日 scheduled → needsMarkInProgress', () => {
    ...
    expect(actions).toEqual([
      { itineraryId: 'i1', kind: 'needsMarkInProgress', at: localNoon(2026, 6, 15) },
    ])
  })

  it('TZ=Asia/Shanghai (UTC+8)：now=次日 2026-07-16 → scheduled 行程 → needsMarkExpired', () => {
    process.env.TZ = 'Asia/Shanghai'
    const actions = reconcileItineraryStatuses(
      [base()],
      localNoon(2026, 6, 16),
    ...
    expect(actions).toEqual([
      { itineraryId: 'i1', kind: 'needsMarkExpired', at: localNoon(2026, 6, 16) },
    ])
  })

  it('TZ=Asia/Shanghai (UTC+8)：now=前一日 2026-07-10 → scheduled 行程 → 跳过（未来）', () => {
    process.env.TZ = 'Asia/Shanghai'
    const actions = reconcileItineraryStatuses(
      [base()],
      localNoon(2026, 6, 10),
  ...
  it('TZ=Pacific/Auckland (UTC+12)：行程日=2026-07-15 本地 → 当日判定一致', () => {
    ...
    // 关键对称性：Auckland 当日与 Shanghai 当日得到同样结果（行程 + now 都是本地正午）
    expect(actions).toEqual([
      { itineraryId: 'i1', kind: 'needsMarkInProgress', at: localNoon(2026, 6, 15) },
    ])
  })
  ...
  it('TZ=America/New_York (UTC-4 夏令时)：行程日=2026-07-15 本地 → 同样归类为当日', () => {

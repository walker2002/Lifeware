...
/**
 * 约定（Appointment）—— 未来日历上一次性的事件安排（[026]，Cycle 模式）
 *
 * 与 Timebox 区别：Timebox 是今日可被 AI 重排的执行格；Appointment 是用户对未来
 ...
 * 时间驱动的 transition（→in_progress, →expired）由 reconcileAppointmentStatuses()
 * 在页面 server component 加载时触发（零 cron）。
 */
export interface Appointment {
  id:             USOM_ID
  status:         AppointmentStatus         // 存储：5 态枚举
  title:          string                  // 活动事件名称

 ...
 * 3. 不同 objectType 同名字段不冲撞（timebox.title ≠ itinerary.title）。
 *
  ...
  fs.writeFileSync(
    ...
    yamlFrom({
      ...
      field_metadata: {
        ...
        itinerary: { title: { type: 'string' }, detail: { type: 'string' } },
      },
    }),
  )
})
...
describe('[026] T23: field_metadata 嵌套结构守卫（validator 启发式单元）', () => {
  ...
  it('不同 objectType 同名字段不冲撞（timebox.title ≠ itinerary.title）', () => {
    const out = execSync(
      `cd ${PROJECT_ROOT} && npx tsx -e "const yaml=require('yaml');const fs=require('fs');const m=yaml.parse(fs.readFileSync('src/domains/timebox/manifest.yaml','utf-8'));const fm=m.field_metadata;const tb=fm.timebox?.title;const it=fm.itinerary?.title;console.log('TB_TYPE='+tb?.type);console.log('IT_TYPE='+it?.type);console.log('TB_KEYS='+Object.keys(fm.timebox).length);console.log('IT_KEYS='+Object.keys(fm.itinerary).length);"`,
      { encoding: 'utf-8', shell: '/bin/bash' },
    )
    expect(out).toMatch(/TB_TYPE=string/)
    expect(out).toMatch(/IT_TYPE=string/)
    // 各自至少 1 字段（timebox 11+ 个，itinerary 5 个）
    expect(out).toMatch(/TB_KEYS=\d+/)
    expect(out).toMatch(/IT_KEYS=\d+/)
    // 不存在 C-flat 错误（嵌套格式）

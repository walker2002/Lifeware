...
2. 通过 manifest 的 `intent_triggers` 声明意图路由上下文，供 Intent Engine 进行有界分类
3. 通过 manifest 的 `lifecycle` 声明对象生命周期定义和状态跃迁规则，供 State Machine 执行校验
4. 对意图提案进行合法性校验（`onValidate`）
5. 响应事件，返回派生指标与建议（`onEvent` / `onActionSurfaceRequest`）
6. 声明出站推送意图（`onOutboundRequest`，可选）——返回推送声明，由 Connector Runner 执行实际 IO

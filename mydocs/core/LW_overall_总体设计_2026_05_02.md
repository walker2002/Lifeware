...
- 增加附录 TODO，可能下一步的核心扩展设计，非MVP考虑

**变更记录（2026_05_14）**：

- 明确 AI 意图驱动的四大机制（具体行动、综合查询统计、教练式指导）
- 补充 Domain manifest `intent_triggers` 字段规范，供 Intent Engine 路由分类使用
- 新增 Intent Engine 两阶段处理模型：阶段 A 路由分类（解释型）+ 阶段 B 字段补全（合约型）
- 新增三类边界情况处理（低置信度澄清、跨 Domain 拆解、斜杠快捷 fast path）
- 新增基于 template_form 和 AI Markdown 的工作流设计
- State Machine 架构修正：生命周期声明下沉到 Domain manifest `lifecycle` 字段，State Machine 降级为通用生命周期执行器
- Orchestrator 新增跨 Domain 意图拆解职责

**变更记录（2026_05_29）**：

- 新增 CNUI Domain Surface Ownership 设计约束 — CNUI surface 组件和数据逻辑归属于 domain，公共层通过 CnuiSurfaceRegistry 动态发现和路由

**变更记录（2026_05_20）**：

- 新增 Context Engine（数据规划引擎）作为 Nexus 第五大核心组件
- Domain Plugin 从"四钩子、三禁止"单轨模型升级为双轨模型：
  - Reactive Track（Hooks）：被动约束系统（不变）
  - Generative Track（Handlers）：主动规划系统（新增）
  - Context Providers：受控共享接口（新增）
- 新增 Generative Path 执行链路
- AI 参与位置新增 Handler（Domain Plugin 范畴内）
- Orchestrator 新增路径识别职责（Reactive vs Generative）
...
│
├── 数据规划层（为生成型操作组装上下文）
│   └── Context Engine（上下文引擎）← 读取 manifest，解析 Context Capability，组装 GenerationRequest
│
├── 业务逻辑层（编排与决策）
│   ├── Rule Engine（规则引擎）
│   └── State Machine（状态机）
...
│   └── AI Runtime（AI 运行时）← 统一 LLM 路由/Session/Token/CN-UI 基础设施，依赖注入到 Handler
│
└── Orchestrator（编排器）
    └── 跨层流程调度，含 Reactive/Generative/Query 三路径识别，不含业务逻辑，不写状态，不参与 AI
```
...
- **跨 Domain 意图拆解**：当 Intent Engine 识别出单一用户输入涉及多个 Domain 时，Orchestrator 将其拆解为多个 `StructuredIntent` 并按顺序处理（处理顺序依赖由 Orchestrator 管理）
- **异常处理**：捕获链路上任意步骤的异常，执行重试 / 回滚 / 降级
- **人工决策暂停点**：Rule Engine 检测到冲突需人工确认时，Orchestrator 挂起执行链，等待用户响应后继续
- **场景编排**：为不同入口（对话输入 / 模板表单 / AI Markdown 确认 / 时间触发 / Bridge Layer 请求）构建不同的执行链路
- **Generative Path**（生成型操作）：Orchestrator 识别 manifest 中有 `generation_actions` 条目 → 委托 Context Engine 组装 GenerationRequest → 准备 Session（如需）→ 将 AI Runtime 注入 Handler 调用 `handler.onGenerate(request, aiRuntime)` → Handler 自主调用 AI → 输出 GenerationResult（或 CN-UI Payload）→ 经 Rule Engine 验证 → 用户确认 → State Machine 批量执行
- **Query Path**（查询型操作，新增）：Orchestrator 识别 manifest 中有 `query_actions` 条目 → 委托 Context Engine 组装查询数据 → 根据复杂度分为两条子路径：Shortcut Path（简单展示型：Orchestrator 直接组装只读 CN-UI）或 Handler Path（复杂分析型：调用 Handler.onQuery 生成文字回答）→ Orchestrator 记录查询摘要到 Session → Session 保持 active。Query Path **不经过** Rule Engine 和 State Machine

### 设计约束

- **不含任何业务逻辑**：不判断意图合法性，不生成提案，不参与 AI 调用
- **不写状态**：所有状态变更仍由 State Machine 执行
- **不参与 AI**：AI 调用由 Handler 内部通过注入的 AI Runtime 自主发起，Orchestrator 只做调度（注入 AI Runtime 参数、传递结果），不直接调用 `aiRuntime.generate()`
...
**生成型路径**（用户请求 AI 生成方案，如"帮我安排今天"）：

```
用户输入（意图为生成型操作）
    ↓ Orchestrator 识别 manifest.generation_actions 中有对应 action
Context Engine（读取 manifest 声明，解析 Context Capability，组装 GenerationRequest）
    ↓
Domain Handler（接收 GenerationRequest，执行 AI 编排/算法，输出 GenerationResult）
    ↓
Rule Engine（验证 GenerationResult 中的 proposalSet）
    ↓
Presentation Layer（Markdown 等格式展示，用户编辑/确认）
    ↓
重新解析 → Rule Engine（二次验证）
    ↓
State Machine（批量执行确认后的 proposal）
    ↓
...
```
用户输入（意图为查询型操作，如"看看我的习惯"）
    ↓ Orchestrator 识别 manifest.query_actions 中有对应 action
Context Engine（读取 manifest 声明，解析 context_capabilities，组装查询数据）
    ↓
    ├── Shortcut Path（简单展示型查询）
    │   Orchestrator 直接组装只读 CN-UI Payload（不经过 Handler）
    │   → 返回客户端渲染
    │
    └── Handler Path（复杂分析型查询）
        Domain Handler.onQuery(context, aiRuntime)
        → Handler 调用 AI 生成分析文字或加工数据
        → 返回文字回答或 CN-UI
    ↓
Orchestrator 记录查询摘要到 Session（通过 Memory Framework API）
    ↓
Session 保持 active（强制 multi_turn）
（无 Rule Engine / State Machine / Event Bus）
```

> Query Path 详细设计参见 `mydocs/dev/LW_Query_Path_Design_v2.md`
...
1. **具体行动信息输入/修改**：直接用自然语言输入明确指令（增加、修改、删除），或通过快捷指令、功能菜单启动具体功能 → **Contract Path**
2. **综合信息查询和统计**：自然语言查询用户数据，包括筛选详细信息和统计信息，可生成数据报告模板和数据查询表单 → **Query Path**（新增，详见 `mydocs/dev/LW_Query_Path_Design_v2.md`）
3. **教练式指导**：针对某个主题、数据或磁贴信息进行深度交流，提供教练式指导 → **未来版本设计**

几种形式可以相互转换，例如执行综合查询后，针对数据进行教练式对话。Query Path 的 Session 机制天然支持这种转换——查询结果作为上下文保持在 Session 中，后续对话可无缝衔接。
...
用户输入
  ↓
[阶段 A] 路由判断：这个意图属于哪个 Domain？哪个 action？哪条路径？
          → 本质是分类问题，AI 必须参与，天然是解释型
          → 输出空间有界：只能选已注册的 Domain 和 action
          → 路径类型由 action 匹配 manifest 的 actions / generation_actions / query_actions 决定
          → Domain manifest 的 intent_triggers + query_actions 作为分类上下文
  ↓
[阶段 B] 字段补全：StructuredIntent 的 fields 是否完整？
          → 本质是结构化提取，AI 辅助，输出类型确定
          → 读取 Domain manifest 的 required_fields
  ↓
StructuredIntent（targetDomain + action + pathType + fields 全部确定）
  ↓
后续链路根据 pathType 路由：
  - contract → Rule Engine → State Machine
  - generative → Context Engine → Handler.onGenerate → Rule Engine → State Machine
  - query → Context Engine → Shortcut Path 或 Handler.onQuery（无 Rule Engine / State Machine）
```

阶段 A 属于"解释型执行"，需要做判断，AI 必须参与；阶段 B 以后根据路径类型不同走不同链路——Contract Path 和 Generative Path 属于"合约型执行"，Query Path 属于"只读查询型执行"（不经过 Rule Engine 和 State Machine）。
...
```
输入：用户原始 input
上下文：所有已注册 Domain 的 intent_triggers 列表 + query_actions 列表

AI 任务：
  从 N 个 Domain × M 个 action 的候选集中，
  选出最匹配的 (targetDomain, action, pathType)，
  输出置信度 0-1

输出：
  { targetDomain: 'habits', action: 'list_active_habits', pathType: 'query', confidence: 0.92 }
  或
  { targetDomain: 'habits', action: 'create_habit', pathType: 'contract', confidence: 0.95 }
  或
...
```
用户: 输入Intent
  - 选择如下三种意图输入方式：自然语言、快捷方式（功能菜单的快捷方式，例如："/habits: create_habit"）、
    功能菜单（兜底选择）
  - 其中快捷方式（未加任何其他指令）、功能菜单可直接触发 template_form 表单录入模式
...
APP: 展现信息
  ...
  - 获取 action 相关合适的 Markdown 模板文件，将 StructuredIntent 按照模板的形式展示
  - 提示用户需要补全的信息

用户: 确认意图
  - 可以选择编辑信息(AI Markdown 形式)来补全
  - 确认信息（意图确认）
```
...
### 基于 template_form 和 AI Markdown 的工作流

对于复杂的创建/规划操作（如创建一组 OKR + 关联 KR + 关联任务），通过读取附件生成 Markdown 文档或者用户直接编辑 Markdown 文档比逐步表单更适合用户协同编辑和 AI 辅助。

**Markdown 处理流程**：

```
AI 生成 Markdown（包括解析上传的附件生成的 Markdown）
...
**注意**：如果一个 Markdown 文件产生多个相互依赖的 StructuredIntent（如先创建 Objective，再创建引用该 Objective ID 的 KeyResult），Orchestrator 需要处理顺序依赖。MVP 建议先限制 Markdown 工作流只产生单一 Domain 的操作，跨 Domain 批处理留后续版本。
...
- 依赖 Domain manifest：读取 `required_fields`、`intent_triggers`（路由上下文）
...
**通用生命周期执行器**，根据 Domain manifest 声明的生命周期规则和触发条件，管理所有 USOM 对象的状态跃迁。State Machine 自身不知道 Habit 有什么状态，它只知道"给我一个合法的跃迁声明，我来执行"。

### 职责

- **状态管理**：根据 Domain manifest `lifecycle` 声明的跃迁规则，变更各对象的生命周期状态，拒绝非法状态跃迁
...
State Machine 不得硬编码任何 Domain 特定的状态流转规则。每新增一个 Domain，State Machine 不需要被修改——只需在 Domain manifest 中声明 `lifecycle` 即可。这确保了 Domain Plugin 的可扩展性承诺不被破坏。
...
│  ◀── Domain manifest（required_fields + intent_triggers）  │
...
┌─────────────────────────────────────────┐
│           State Machine                  │
│  ◀── Domain manifest（lifecycle 跃迁声明）│
...
Context Engine 与 Handler 的依赖关系（Generative Path）：
         │
         ▼ (Orchestrator 识别为 generative 路径时)
┌─────────────────────────────────────────┐
│           Context Engine                │
│  ◀── Domain manifest（generation_actions）│
│  ◀── Context Registry（ContextCapability）│
│  ──▶ GenerationRequest                  │
└────────┬────────────────────────────────┘
         │ GenerationRequest
         ▼
...
└────────┬────────────────────────────────┘
         │ GenerationResult
         ▼ (经 Rule Engine 验证 → 用户确认 → State Machine 执行)
```
...
Domain 具备双轨能力模型：
- **Reactive Track（Hooks）**：声明式规则与纯计算函数的集合，是被动约束系统，只能被 Nexus 在固定时机通过钩子调用。
- **Generative Track（Handlers）**：主动计算单元，接收 Context Engine 组装的数据，执行生成型操作（如 AI 编排），输出结构化方案。

Context Providers 为 Handler 提供跨域数据的受控共享接口。Hooks 和 Handlers 共存于每个 Domain，互不干扰。

## 4.2 能力边界（双轨模型）

#### Reactive Track — Hooks（约束系统）

**四种能力**：
1. 通过 manifest 声明它关心的对象类型与结构完整性约束
2. 通过 manifest 的 `intent_triggers` 声明意图路由上下文
3. 通过 manifest 的 `lifecycle` 声明对象生命周期定义
4. 对意图提案进行合法性校验（`onValidate`）、响应事件（`onEvent`）、返回行动候选（`onActionSurfaceRequest`）、声明出站推送意图（`onOutboundRequest`）

**三条禁令**：
1. **不能写入状态**
2. **不能主动运行**
3. **不能访问其他 Domain 的内部数据**

#### Generative Track — Handlers（规划系统）

**Handler 约束**：
- Handlers MAY 调用 AI（Domain 内唯一允许 AI 参与的位置）
- Handlers MUST NOT 直接访问 Repository（数据由 Context Engine 提供）
- Handlers MUST NOT 写入状态（输出为 GenerationResult，经 Rule Engine 验证后由 State Machine 执行）
- Handlers MUST NOT 直接触发事件

#### Context Providers（受控共享接口）

**Provider 约束**：
- 仅允许读取、投影、轻量聚合
- 禁止 planning、决策、复杂计算、调用 AI
- 输出必须通过 Zod schema 校验
...
#### CNUI Surface Ownership

CNUI surface 组件、handler 和数据逻辑归属于对应 domain。公共层通过 `CnuiSurfaceRegistry` 机制动态发现和路由，不再硬编码 domain 特定的组件导入或 if/else 分支逻辑。新增 domain 添加 CNUI surface 时公共层零改动。Each surface declares its handler in `manifest.yaml` 的 `cnui_surfaces` 块，action-to-surface mapping 通过 `response_type` 字段定义在 `intent_triggers` 中。

---

## 4.3 统一插件接口：四钩子模型 + Manifest 自描述

### Manifest 自描述能力

Domain manifest 是 Nexus 组件运作的结构化知识来源，包含以下关键字段：

```yaml
# example: habit/manifest.yaml
domain_id: habits
version: "1.0.0"

# 意图路由上下文（供 Intent Engine 阶段 A 使用）
intent_triggers:
  - action: create_habit
...
# 对象生命周期定义（供 State Machine 使用）
lifecycle:
  - object: habit
    states: [draft, active, suspended, archived]
    transitions:
      - { from: draft, to: active, trigger: intent }
      - { from: active, to: suspended, trigger: intent }
      - { from: active, to: archived, trigger: intent }
      - { from: suspended, to: active, trigger: intent }
    irreversible: [archived]

# 字段定义（供 Intent Engine 阶段 B 使用）
required_fields:
  - name: name, type: string, required: true
  - name: frequency, type: RecurrenceRule, required: true
  - name: start_date, type: date, required: false

# Markdown 模板定义（供 AI Markdown 工作流使用）
markdown_template: "templates/create_habit.md"
```

**关键约束**：
- `intent_triggers` 必须是结构化声明（不是自由格式文本），供 Intent Engine 做有界分类
- `lifecycle` 定义对象的状态空间和合法跃迁，State Machine 据此校验，自身不硬编码任何 Domain 规则
- 添加新 Domain 时，Nexus 组件（Intent Engine、State Machine）不需要修改，只需注册新的 manifest 声明
...
- **Domain Plugin Handlers**：领域特定的生成型 AI 逻辑（如时间盒智能编排），通过 Handler 接收 Context Engine 组装的数据，输出 GenerationResult
...
以下内容尚未纳入设计方案，也不影响 MVP设计，但是作为后续考虑的重点。

## 1. 偏差建模 Deviation

### 问题来源

现在你只是记录完成，这是不够的，应该显式建模：
...
将 Deviation 作为重要大的“可分析资产”，这是“成长系统”的核心数据



### 设计思考

#### ✅ 层 1：State Machine（记录事实）
...
```
timeEstimationPattern: {
  avgDeviationRate: number     // 平均偏差比例
  variance: number             // 稳定性
  bias: 'underestimate' | 'overestimate'
  confidence: number
}
```
...
## 👉 Deviation Aggregator（应该在 Memory 内）

需要一个内部模块：

```
Memory Framework
 └── Deviation Aggregator
```
...
[Memory Framework]
   ...
   ↓
[Rule Engine / Action Surface / Intent Engine]
```
...
> **Deviation 不是一个“模块”，而是一种“贯穿系统的反馈信号”。**



## 2. Domain Proposal 回流（进阶能力）

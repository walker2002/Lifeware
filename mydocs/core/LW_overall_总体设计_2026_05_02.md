...
│
└── Orchestrator（编排器）
    └── 跨层流程调度，含 Reactive/Generative 路径识别，不含业务逻辑，不写状态，不参与 AI
```
...
1. **具体行动信息输入/修改**：直接用自然语言输入明确指令（增加、修改、删除），或通过快捷指令、功能菜单启动具体功能
2. **综合信息查询和统计**：自然语言查询用户数据，包括筛选详细信息和统计信息，可生成数据报告模板和数据查询表单
3. **教练式指导**：针对某个主题、数据或磁贴信息进行深度交流，提供教练式指导
...
[阶段 A] 路由判断：这个意图属于哪个 Domain？哪个 action？
          ...
          → Domain manifest 的 intent_triggers 作为分类上下文
  ↓
...
阶段 A 属于"解释型执行"，需要做判断，AI 必须参与；阶段 B 以后属于"合约型执行"，是必须遵循的确定性流程。

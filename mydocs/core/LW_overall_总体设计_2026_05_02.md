...
- **Generative Path**（生成型操作）：Orchestrator 识别 manifest 中有 `generation_actions` 条目 → 委托 Context Engine 组装 GenerationRequest → 调用 Domain Handler → Handler 输出 GenerationResult → 经 Rule Engine 验证 → 用户确认 → State Machine 批量执行

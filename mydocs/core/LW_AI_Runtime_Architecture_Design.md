...
> - 完整执行链路
>
> 文档版本：V3.1 | 编制日期：2026-05-23 | 密级：内部公开
>
...
- **判断8：AI Runtime 边界**。所有 AI 调用的统一入口，包括 Intent Engine 的 LLM 调用。Intent Engine 不直接调用 `/lib/llm/`，通过 AI Runtime 的 LLMGateway 统一路由
- **判断9：Provider 策略**。兼容现有中国 Provider（通义千问 DashScope / DeepSeek / 智谱 Zhipu）通过 OpenAI 兼容接口适配，新增 Anthropic / Ollama 通过 Vercel AI SDK 支持。现有 `/lib/llm/` 逐步废弃
- **判断10：Memory 集成**。AI Session 先行，Memory Framework L1 是 MVP 前置依赖。Phase 2 再引入 L2 摘要和长期记忆
- **判断11：开发策略**。AI Runtime 基础骨架与 CN-UI 并行开发，3 Sprint 分阶段交付
...
- **Intent Engine 的 AI 调用也通过 AI Runtime**：Intent Engine 的意图分类和字段补全调用通过 `LLMGateway` 统一路由，taskType 为 `intent_routing` / `field_extraction`。Intent Engine 不直接调用 `/lib/llm/`，而是通过 AI Runtime 的 LLMGateway 完成所有 LLM 调用
...
| 统一 Provider 接口 | 一套 API 调用 OpenAI/Anthropic/Google/Ollama 等；中国 Provider（DashScope/DeepSeek/智谱）通过 OpenAI 兼容接口适配 | 模型切换无代码改动 |
...
#### 5.1.0 LLMGateway 与现有 /lib/llm/ 的关系

**迁移策略**：LLMGateway 包装现有 `/lib/llm/` 的能力，而非从零重写：

| 层次 | 现有代码 | LLMGateway 后 |
|------|---------|--------------|
| Provider 配置 | `/lib/llm/config.ts`（DashScope/DeepSeek/智谱/OpenAI） | LLMGateway 复用配置，新增 Anthropic/Ollama |
| 调用能力 | `/lib/llm/client.ts`（OpenAI SDK 封装的 `chat()` 函数） | LLMGateway 包装现有 `chat()` + 新增 Vercel AI SDK 调用 |
| 适配器 | OpenAI 兼容接口（DashScope/DeepSeek/智谱均走 OpenAI SDK） | 保持不变，中国 Provider 继续通过 OpenAI 兼容接口适配 |

**关键约束**：
- 现有 `/lib/llm/` 在 LLMGateway 上线后逐步废弃，新代码统一通过 LLMGateway 调用
- Intent Engine（`ai-parser.ts`）从直接调用 `chat()` 迁移到通过 AI Runtime 的 LLMGateway 调用
- Handler 的 AI 调用通过 `aiRuntime.generate()` / `aiRuntime.stream()` 间接使用 LLMGateway
...
interface LLMGateway {
  ...
  call(providerName: string, request: AIGenerateRequest): Promise<AIGenerateResponse>
}

// Gateway 内部根据 providerName 路由到对应 Provider 配置
type ProviderRoute = {
  provider: string   // 如 'dashscope' | 'anthropic' | 'ollama'
  model: string      // 如 'glm-5.1' | 'claude-sonnet-4-6'
}

// 路由策略：从 UserSettings.llmConfig 读取，不在代码中硬编码
type AITaskType =
  | 'intent_routing'      // 意图分类，轻量快速
  | 'field_extraction'    // 字段补全提取，轻量
  | 'content_generation'  // 复杂内容生成（强模型）
  | 'summary'             // 摘要生成，中等
  | 'cn_ui_revision'      // CN-UI AI 修订，中等

// 默认路由配置（可通过 UserSettings.llmConfig 覆盖）
...
**AITaskType 与现有 AI 调用场景的映射**：

| 调用位置                   | taskType             | 现有代码位置                         | 说明                     |
| -------------------------- | -------------------- | ------------------------------------ | ------------------------ |
| Intent Engine（意图路由）  | `intent_routing`     | `nexus/core/intent-engine/ai-parser.ts` 的意图分类调用 | 每次用户输入都触发       |
| Intent Engine（字段补全）  | `field_extraction`   | `ai-parser.ts` 的字段提取调用        | Intent 字段不完整时触发  |
| Domain Handler（内容生成） | `content_generation` | `domains/*/handlers/*.ts` 的 `onGenerate()` | 按 Handler 触发          |
| 总结（摘要生成）           | `summary`            | `onGenerate()` 中的 summary 类 action | 对内容进行总结           |
| CN-UI AI 修订              | `cn_ui_revision`     | CnuiSurface 内用户请求修改时         | 用户在 CN-UI 内输入时触发 |
...
包括 id、domainID、action、Session模式（'single_shot' | 'conversational'）、状态以及一些运行时状态，如ProposalState、Conversation状态（压缩信息+最近10条）、CNUIStatus等
...
CnuiManager 负责管理对话中 CN-UI 界面的全生命周期。每个 CN-UI 界面是对话流中的一个独立交互区域（如一张习惯创建卡片、一个时间盒规划列表）。

```typescript
interface CnuiManager {
  // 创建 CnuiSurface
  createCnuiSurface(config: CnuiSurfaceConfig): Promise<CnuiSurface>

  // 更新 CnuiSurface（数据变更时）
  updateCnuiSurface(cnuiSurfaceId: string, update: CnuiSurfaceUpdate): Promise<void>

  // 关闭 CnuiSurface
  closeCnuiSurface(cnuiSurfaceId: string): Promise<void>

  // 处理用户交互事件
  handleEvent(event: CnuiEvent): Promise<CnuiResponse>
...
interface CnuiSurfaceConfig {
  ...
  domainId: string
  action: string
  initialData?: Record<string, unknown>   // 预填充数据
  layout: CnuiLayout
}
...
#### 10.3.2 Component Catalog

Component Catalog 是 CN-UI 的安全边界——Agent 只能使用 Catalog 中预注册的组件类型，不能创建任意组件。

#### 10.3.3 CnuiEventBus

CnuiEventBus 负责处理用户在 CnuiSurface 上的交互事件，并将事件路由到对应的 Handler 处理。
...
interface CnuiResponse {
  ...
  extractedData?: Record<string, unknown>   // 提取的结构化数据
}
```

### 10.4 Component Catalog 设计
...
### 10.5 CnuiSurface 管理与生命周期

#### 10.5.1 CnuiSurface 生命周期状态机
...
#### 10.5.2 CnuiSurface 渲染流程

```
1. Handler 调用 AI Runtime 生成响应
2. AI Runtime 判断 responseMode === 'cnui'
3. LLM 生成 CN-UI Payload（JSON 描述 CnuiSurface 结构和数据）
4. Schema 校验 + 修复性重试（见 10.5.3）
5. CnuiManager.createCnuiSurface(config, payload)
6. CnuiSurfaceStore 写入 Surface 状态
7. 客户端通过 CN-UI Renderer 渲染 CnuiSurface
8. 用户看到对话流中出现交互式组件
```

#### 10.5.3 CN-UI Payload Schema 校验与修复
...
#### 10.5.4 CN-UI Payload 的 LLM 生成机制

Handler 的 systemPrompt 中注入 Component Catalog 描述（可用组件列表 + Props Schema），LLM 据此生成符合 CnuiSurface 规范的 JSON Payload：

```
1. Handler 从 CnuiComponentCatalog 获取当前 cnuiSurfaceType 可用的组件列表及 Props Schema
2. Handler 将组件描述注入 systemPrompt（作为生成约束）
3. Handler 通过 aiRuntime.generate() 调用 LLM，传入 structuredOutput 的 Zod Schema 约束输出格式
4. LLM 在 systemPrompt 约束 + Zod Schema 双重约束下生成 CnuiPayload
5. AI Runtime 内部执行 Schema 校验 + 修复性重试（见 10.5.3）
```

**关键约束**：
- LLM 只能引用 Catalog 中预注册的组件，不能创建任意组件
- Zod Schema 确保 Payload 结构类型安全
- Handler 完全控制 prompt 内容和 schema 定义，AI Runtime 只提供校验基础设施

### 10.6 CnuiEvent 回传机制

CnuiEvent 回传是整个对话闭环的关键。用户在 CnuiSurface 上的每一个操作都需要被捕获、处理，并可能影响对话的走向。
...
interface FormSubmitEvent extends CnuiEvent {
  type: 'form_submit'
  payload: {
    formData: Record<string, unknown>   // 整个 CnuiSurface 的当前数据
  }
}
```
...
```
1. 用户在 CnuiSurface 上操作（如点击"确认"按钮）
2. CN-UI Renderer 捕获操作，构造 CnuiEvent
3. Event 通过 EventBus 发送到 CnuiManager.handleEvent(event)
4. CnuiManager 根据 event.cnuiSurfaceId 找到对应的 CnuiSurface 和 Session
5. CnuiManager 更新 CnuiSurface 的 dataModel
6. 如果 Event 是"确认/提交"类型：
   a. 提取 CnuiSurface.dataModel 作为结构化数据
   b. 复用现有 Proposal 确认流程：数据 → Rule Engine → State Machine
      （不新增独立确认路径，与现有 executeGenerativeConfirmation 复用同一链路）
   c. 校验通过：关闭 CnuiSurface，数据写入系统
   d. 校验失败：返回 validation_error，CnuiSurface 显示错误信息
7. 如果 Event 是"输入变更"类型：
   a. 更新 CnuiSurface 状态
   b. 可选：触发实时校验或联动更新
```

### 10.7 CnuiSurface Store

#### 10.7.1 定义

**CnuiSurface Store 是 CnuiSurface 的运行时状态容器**，负责存储对话中每个交互式 UI 卡片（Surface）的当前数据状态。它是连接"用户在 UI 上的操作"和"AI 后续处理"的数据桥梁。

#### 10.7.2 MVP 设计原则：最小可用

MVP 阶段的 Surface Store 遵循**最小可用**原则——只做必须的事，不过度设计：

| 能力 | MVP | Phase 2 |
|:-----|:----|:--------|
| 状态存储 | 内存 Map，key=cnuiSurfaceId, value=dataModel | 持久化存储，支持跨会话恢复 |
...
interface CnuiSurfaceStoreAPI {
  ...
  set(cnuiSurfaceId: string, data: CnuiSurfaceData): void
  delete(cnuiSurfaceId: string): void   // Session 关闭时批量调用
}
```

> **Phase 2 扩展预留**：当需要持久化、并发控制、历史快照时，将 `Map` 替换为实现了 `CnuiSurfaceStoreAPI` 的持久化实现，上层调用方无需改动。

### 10.8 CN-UI 与 Session 的整合
...
interface CnuiMessage extends ChatMessage {
  ...
  dataSnapshot?: Record<string, unknown>  // 完成时的数据快照
}
```

### 10.9 场景示例

#### 10.9.1 场景一：习惯创建（参数补全型 CN-UI）
...
#### 10.9.2 场景二：时间盒计划生成（复杂交互型 CN-UI）
...
function CnuiRenderer({ payload, onEvent }: CnuiRendererProps) {
  ...
  const SurfaceComponent = resolveCnuiSurfaceComponent(cnuiSurfaceType)
...
```
1：AI Runtime 基础骨架
  □ LLMGateway（兼容现有中国 Provider + 新增 Anthropic/Ollama）
...
5：TimeboxTimeline CN-UI
  ...
  □ confirm 回流逻辑（复用现有 Proposal 确认流程：Rule Engine → State Machine）

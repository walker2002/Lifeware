...
DomainPlugin {
  ...
  // 钩子三：行动切面候选（原 onTileRequest，升级为 onActionSurfaceRequest）
...
- 输出永远是声明，不是执行——`suggestions` 是候选，`metrics` 是计算结果，Core 决定是否采纳和如何执行
- 函数签名固定，第三方开发者只需理解这三个签名，无需了解 Core 内部实现
...
**扩展新 Domain**：实现三钩子函数 + 编写 manifest + 注册到 Core，无需改动 Core 代码。

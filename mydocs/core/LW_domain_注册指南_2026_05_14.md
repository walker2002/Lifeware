...
| 输出 | 校验结果 / 指标 / 行动候选 | GenerationResult（方案 + 展示 + 警告） |
| 状态写入 | 禁止 | 禁止（输出经 Rule Engine 验证后由 State Machine 执行） |

### 接口定义

Handler 必须实现 `DomainHandler` 接口：

```typescript
import type {
  DomainHandler, GenerationRequest, GenerationResult,
  GeneratedProposal, ProposalSet, PresentationPayload, Warning
} from '@/usom/types/process'

export class MyActionHandler implements DomainHandler {
  async handle(request: GenerationRequest): Promise<GenerationResult> {
    // request.intent — 用户参数（StructuredIntent）
    // request.contexts — Context Engine 组装的系统数据
...
export const myDomainHandlers: Record<string, DomainHandler> = {
  create_smart_plan: new MyActionHandler(),
  adjust_remaining:  new MyAdjustHandler(),
}
```
...
□ Handler 不直接触发事件
□ Handler 必须包含降级路径（AI 失败时退化为规则编排）
...
Handler（Step 9，仅生成型 Domain）
  ...
  □ Handler 不写入状态（输出 GenerationResult）
  □ Handler 包含 AI 失败时的降级路径

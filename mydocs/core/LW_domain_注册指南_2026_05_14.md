...
**注**：Step 9–10 仅在 Domain 需要生成型能力时执行。Step 11–12 仅在 Domain 需要查询型能力时执行。简单展示型查询（Shortcut Path）只需完成 Step 11 即可，不需要 Handler。普通 Domain 只需完成 Step 1–8。
...
# ── 区块 G（可选）：查询型操作（Orchestrator + Context Engine 读取）──
...
query_actions:
  - action: list_active_items
    ...
    context_capabilities:
      - capability: active_items
        query: 'active_for_user'
    examples:
      - "看看我的列表"
      - "有哪些项目"

  - action: item_statistics
    description: "查询统计信息"
    response_mode: text              # text = Handler Path（需要 Handler.onQuery 生成分析文字）
    context_capabilities:
      - capability: item_logs
        query: 'logs_for_period'
      - capability: item_streaks
        query: 'current_streaks'
    examples:
      - "统计一下完成情况"
      - "坚持多久了"

  # 注意：query_actions 不声明 session_mode，系统强制默认为 multi_turn


# ── 区块 H（可选）：生成型操作（Context Engine + Orchestrator + AI Runtime 读取）──
# 完整声明参见 Step 9 的 generation_actions 示例
```
...
Handler 需要在 manifest 的 `generation_actions` 块中声明，新增字段包括：

```yaml
# ── 区块 G（可选）：生成型操作（Context Engine + Orchestrator + AI Runtime 读取）──
generation_actions:
  - action: generate_daily_timeboxes
    description: "生成一天的时间盒安排"
    session_mode: conversational       # single_shot | conversational
    response_mode: cnui                # text | cnui
    cnui_surface: timebox-list         # CN-UI Surface 类型（response_mode=cnui 时必填）
    cache_ttl_minutes: 30              # 缓存有效期（可选）
    context_capabilities:
      - capability: active_tasks
        query: 'active_for_date'
        params:
          date: '${intent.fields.date}'
      - capability: active_habits
        query: 'active_for_user'
      - capability: energy_state
        query: 'current_state'
  ...
  - action: create_habit
    ...
    context_capabilities:
      - capability: habit_categories
        query: 'all'
      - capability: user_energy_pattern
        query: 'default'

  - action: generate_summary
    description: "生成每日总结"
    session_mode: conversational
    response_mode: text               # 纯文本输出，不使用 CN-UI
    cache_ttl_minutes: 0              # 不缓存
    context_capabilities:
      - capability: daily_events
        query: 'events_for_date'
        params:
          date: '${intent.fields.date}'
```
...
| `session_mode` | 是 | `single_shot`（不展示对话输入框）/ `conversational`（多轮对话） |
| `response_mode` | 是 | `text`（纯文本/markdown）/ `cnui`（CN-UI Payload） |
| `cnui_surface` | response_mode=cnui 时必填 | Component Catalog 中的 Surface 类型 ID |
| `cache_ttl_minutes` | 否 | 结果缓存有效期（分钟），0 或不填表示不缓存 |
| `context_capabilities` | 是 | Handler 需要的 Context Provider 依赖列表 |
...
query_actions:
  ...
  - action: habit_statistics
    ...
    response_mode: text
    context_capabilities:
      - capability: habit_logs
        query: 'logs_for_period'
      - capability: habit_streaks
        query: 'current_streaks'
    examples:
      - "习惯统计"
      - "跑步坚持多久了"
...
Handler（Step 9，仅生成型 Domain）
  ...
  □ generation_actions 中 session_mode / response_mode / cnui_surface 已正确声明
...
Query Actions（Step 11，仅查询型 Domain）
  ...
  □ response_mode=cnui 的 action 声明了 cnui_surface
  □ context_capabilities 引用的 Provider 已注册
  □ query_actions 未声明 session_mode（系统强制 multi_turn）

...
- **2026_07_12 (refactor)**：[TD-003] timebox 域 OCC（乐观并发控制）POC — `ITimeboxRepository` 新增 `updateFields(id, fields, userId, expectedOccVersion, tx?)` 与 `archive(id, userId, expectedOccVersion, executionRecord?)`（`expectedOccVersion: number` 必填）；OCC atomic UPDATE `WHERE occ_version = expectedOccVersion`（0 rows → 抛 `ConflictError(currentOccVersion, attemptedOccVersion)`）；field-executor 重构 `executeBatch`（validate-all-first + single atomic update + post-write events）；`service.execute()` outer catch 显式 re-throw `ConflictError` 保留 `err.name='ConflictError'`。USOM `Timebox` interface **不暴露** `occVersion`（属 Repository 契约，不污染域对象——USOM 层不感知并发控制语义）。**仅 timebox 域 POC**，5 域（tasks/habits/appointments/cycles/objectives）跨域 OCC → `[[TD-037]]` deferred。详见 CHANGELOG.md `[TD-003]` 段 + `docs/tech-debt/TD-003-edit-timeboxes-toctou.md`（已关闭）。

- **2026_07_11 (refactor)**：[027-A] Phase A — `Task.activityArchetypeId` / `Habit.activityArchetypeId` 由 `USOM_ID?` widen 到 `USOM_ID | null`（3-state：undefined=skip / null=clear / string=set），对齐 DB nullable FK 列（`on delete set null` 已支持 NULL，USOM 类型原先 undefined-only 是 imprecision）。触发原因：[027-A] 全局「清除原型」按钮让 null=clear 在 UI 真正可达，输入类型（CreateTaskInput/UpdateTaskInput/CreateHabitInput/UpdateHabitInput）+ USOM 对象类型须一致支持 null。Timebox/Appointment 对象类型暂未 widen（pre-existing imprecision，登记 neat/后续）。代码：`frontend/src/usom/types/objects.ts` Task(:361) + Habit(:476) + `irepository.ts`。

- **2026_07_06 (refactor)**：[023.12] 三域生命周期语义重构 — `TimeboxStatus` 收窄到 3 态 `planned | logged | cancelled`（原 6 态 `planned | running | overtime | ended | cancelled | logged` 收敛）；`AppointmentStatus` 收窄到 3 态 `scheduled | cancelled | completed`（原 5 态 `scheduled | in_progress | expired | cancelled | completed` 收敛）；`CycleStatus` 收窄到 4 态 `draft | approved | finished | reviewed`（原 5 态 `draft | not_started | in_progress | ended | reviewed` 收敛）。`running`/`overtime`/`ended` 与 `in_progress`/`expired` 改为**读时派生**（`derive-display-status.ts`：timebox 看 now vs startTime/endTime，appointment 看 now 与 startTime 同日历日），时间态不再持久化。`Cycle.startedAt`/`endedAt` 按 AM6 重命名为 `approvedAt`/`finishedAt`（USOM 与 schema 同步）。三域各加 2 条 revert transition：timebox `logged|cancelled→planned`、appointment `cancelled|completed→scheduled`、cycle `reviewed→finished`（单步保留复盘证据）。`AppointmentReverted`/`TimeboxReverted`/`CycleReverted` 三事件加入 manifest subscribed_events。**反向 [026] D2 reversal**：appointment 持久化模式回到读时派生，理据见 design doc §与 [026] D2 reversal 的关系段。`Cycle` 字段语义：cycle `reviewed` 是"复盘锁定"，`reviewed→finished` 单步回退保留 review 证据（与 timebox/appointment 回初态不对称，是有意设计）。schema 同步：timeboxes/appointments 删 5 个时间戳列（startedAt/endedAt/overtimeAt + inProgressAt/expiredAt），cycles rename started_at→approved_at + ended_at→finished_at（迁移 0034，TRUNCATE 三表 + DROP/RENAME）。详见 docs/database-design.md [023.12] 段。
- **2026_07_02 (refactor)**：[022.01] Phase 3 — 移除 Objective/KR 独立状态 — `Objective` 移除 `status: ObjectiveStatus` 字段与 `ObjectiveStatus` 枚举（6 值）；`KeyResult` 移除 `status: KeyResultStatus` 字段与 `KeyResultStatus` 枚举（6 值）。状态权威迁移至 `Cycle.status`（§3.5a）；KR 完成语义由 `progressRate >= 1.0` → `completedAt` 自动设置承载。`paused` 语义永久丢失（迁移有损可接受）。可编辑性由所属 Cycle 状态决定：`draft`/`not_started`/`in_progress`/`ended` cycle → 可编辑；`reviewed` cycle → 只读；删除 Objective 仅限 `draft` cycle + 无 KR 关联守卫
...
- **2026_07_12 (refactor)**：[TD-002] Timebox CNUI handler 批量写语义统一为 partial-success with explicit report（与宪法 §XV.6 一致）— `logTimebox` 分支原"早 break 不回滚"重构为收集 `succeeded[]` / `failed[]` + `result.data.count/succeeded/failed` + `result.error` 含 title + 具体原因；同文件 `createTimebox` / `scheduleProposal` / `adjustRemainingTimeboxes` / `createAppointment` 已遵循同范式，本次收口 5/5 分支一致性。本改动**不**影响宪法 §III 单事务边界（cross-object 复合写的 ACID），仅覆盖 CNUI handler 批量循环调用多个独立写入口的场景——已落库的独立事务不回滚，未落库的修正后重试，由用户决策下一步。
...
- **2026_06_30 ([023] A3.3)**：硬删 `HabitTemplate` / `TemplateHabitItem` 接口（已被 timebox-templates 取代，迁移 0027）
...
### 3.5 OKR：Objective（目标）— (v2 — since 2026-07-02: removed status field)
...
软删除/归档仍由 `discardedAt` / `completedAt` / `archivedAt` 时间戳承载。

| 时间戳 | 语义 |
|---|---|
| `discardedAt` | 软删除时刻，标记 Objective 不再活跃、不参与编排 |
| `completedAt` | Objective 自身完成时刻（与 KR 完成语义解耦，Objective 完成由业务规则触发） |
| `archivedAt` | 历史归档时刻 |

> **v2 变更（2026-07-02，[022.01] Phase 3）**：移除 `status: ObjectiveStatus` 字段与 `ObjectiveStatus` 枚举（6 值）。状态权威统一收敛至 `Cycle.status`（见 §3.5a）。`paused` 语义永久丢失（迁移有损可接受），由 Cycle 状态约束编辑代替。
...
### 3.6 OKR：KeyResult（关键结果）— (v2 — since 2026-07-02: removed status field)

**对象意图**：衡量 Objective 达成程度的可量化指标，是 Task 和 Habit 向上承接的锚点。

> **v2 变更（2026-07-02，[022.01] Phase 3）**：移除 `status: KeyResultStatus` 字段与 `KeyResultStatus` 枚举（6 值）。完成语义由 `progressRate` 承载：`progressRate >= 1.0` → `completedAt` 自动设置（由 `updateProgress` 派生写）。`paused` 语义永久丢失（迁移有损可接受），由所属 Cycle 状态约束编辑代替。软删除/归档仍由 `discardedAt` / `archivedAt` 时间戳承载。
...
interface KeyResult {
  ...
  updatedAt:    Timestamp
  discardedAt?: Timestamp
  // [022.01] 2026-06-26：补齐 KR 生命周期时间戳。
  // v2 (2026-07-02)：completedAt 由 updateProgress 派生写（progressRate >= 1.0），
  // archive() 写 archivedAt。mapper 此前漏映射导致 findById 丢失
...
interface Task {
  ...
  activityArchetypeId?:  USOM_ID | null          // [023] A3.1：nullable FK → activity_archetypes.id（取代旧 energyProfile）；[027-A] widen 到 | null（3-state：undefined=skip / null=clear / string=set）
...
- `activityArchetypeId?: USOM_ID | null`（nullable FK → activity_archetypes.id；[027-A] widen 到 | null 支持 3-state clear）
...
interface Habit {
  ...
  startDate:        DateOnly
  endDate?:         DateOnly
  activityArchetypeId?: USOM_ID | null  // [023] A3.1：nullable FK → activity_archetypes.id（习惯类型归类）；[027-A] widen 到 | null（3-state clear）
...
> **[023.12] Timebox 字段清理**：`startedAt?`/`overtimeAt?`/`endedAt?` 三个时间戳列已 drop（schema.ts 删除 5 个废弃列对应 3 个）；USOM TypeScript interface 同步移除这 3 个可选字段。`executionRecord?`/`loggedAt?` 保留（`Logged` 状态必填）。`status: TimeboxStatus` 类型收窄（见上）。**Date locale 治本**（T14）：`process.env.TZ='Asia/Shanghai'` 在 `frontend/next.config.ts:3` 显式设置，`localDayKey` 类函数不再依赖服务器 locale（避免 CI/prod UTC 偏 8h）。
...
*变更：[TD-003] (2026_07_12) — §3.10 Timebox Repository 接口加 `expectedOccVersion: number` 必填参数（OCC atomic UPDATE 谓词）；USOM `Timebox` interface 不暴露 occVersion 字段（Repository 契约不污染域对象）；详见 CHANGELOG.md `[TD-003]` 段*

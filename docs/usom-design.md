...
interface DetailedExecutionRecord extends Omit<SimpleExecutionRecord, 'mode'> {
  mode: 'detailed'
...
- `AISession`
  ...
  - `updatedAt`: Timestamp
  - `archivedAt?`: Timestamp
  - 生命周期：active → archived → deleted；archived → active(恢复)；active 不可直接 deleted
- `AISessionSummary`（列表摘要）
  - `id`: USOM_ID, `title`: string, `status`: AISessionStatus, `createdAt`: Timestamp, `updatedAt`: Timestamp
- `LLMConfig`
  - `provider`: string, `baseUrl`: string, `apiKey`: string, `defaultModel`: string
- `UserSettings`

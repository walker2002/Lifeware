...
```
DROP 顺序（先 junction 再主表，规避 FK 依赖阻断）
  ├─ DROP TABLE IF EXISTS template_habits;   -- 先 junction（其 template_id FK → habit_templates.id；先 DROP 主表会被此 FK 依赖阻断）
  └─ DROP TABLE IF EXISTS habit_templates;
SELECT count 守护：DROP 前确认两张表 row count（无 seed，预期 0 或仅用户自建）
```

> **R4 修正（autoplan H1，2026-06-30）**：旧表述「junction 的 habit_id 是 RESTRICT → 须先 DROP junction」依据错误。`template_habits.habit_id ON DELETE RESTRICT` 治理的是**删一行 habit** 的行为，与 `DROP TABLE` 无关。真正强制顺序的是 `template_habits.template_id → habit_templates(id)`（CASCADE）——junction 依赖主表，故须先 DROP junction 再 DROP 主表。顺序不变，依据纠正。
...
- **R4 template_habits → habit_templates FK 依赖**：junction `template_habits.template_id` FK 引用主表 `habit_templates(id)`，故 DROP 须先 junction 再主表（§4.3）。若生产有存量 template_habits 行，DROP 前确认（SELECT count 守护）。（旧表述误把 `habit_id RESTRICT` 当作顺序依据，已纠正——RESTRICT 治理删 habit 行，与 DROP TABLE 无关。）

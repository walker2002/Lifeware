...
用户在 CNUI 表单选 Archetype
  ...
  → domain-mutation-service resolveMutationMode('ContentField')
  → Repository.updateFields（原子写，不发业务事件）
...
- **ArchetypePicker（公共化后）**：`src/components/archetype/archetype-picker.tsx`，props `{ value?: string; onChange: (id, archetype?) => void }`，server action `getArchetypes()` 取数，选中后只读展示 `l2Name` + `l1Category` + `EnergyCostAccordion`。
- **接入点（A3.2）**：
  - tasks：`domains/tasks/cnui/surfaces/TaskCreationCard.tsx` + `TaskEditCard.tsx`
  - habits：`domains/habits/components/habit-form.tsx`（被 `HabitCreationCard` 包裹，[019.1] 手写范式）
- **详情只读**：tasks 详情抽屉（`TaskDetailDrawer`）+ habits 详情面板，追加 archetype 只读行（l2Name + L1 标签 + 4 维只读）。
- **optional 语义**：未选时显示「未选择（可选）」，不阻塞提交（D3）。
...
**File Structure**:
- Move: `domains/timebox/components/archetype-picker.tsx` → `src/components/archetype/archetype-picker.tsx`
- Move: `domains/timebox/components/energy-cost-accordion.tsx` → `src/components/archetype/energy-cost-accordion.tsx`
- Modify: `domains/timebox/cnui/surfaces/CreateTimebox.tsx`（及 Drawer 等引用方）改 import 指向 `@/components/archetype`
- Modify: `domains/tasks/cnui/surfaces/TaskCreationCard.tsx` + `TaskEditCard.tsx`（接 ArchetypePicker）
- Modify: `domains/habits/components/habit-form.tsx`（接 ArchetypePicker）
- Modify: tasks/habits 详情面板（只读 archetype 行）
- Modify: `domains/tasks/manifest.yaml` + `domains/habits/manifest.yaml`（field_metadata.activityArchetypeId = ContentField）
- Modify: `docs/usom-design.md` + `docs/database-design.md`（Tier 2）

**Task 边界（3）**：
1. ArchetypePicker + EnergyCostAccordion 公共化（D7）+ timebox 改 import + timebox 回归
2. tasks/habits 表单接入 + manifest field_metadata
3. 详情面板只读展示 + /browse 视觉验证（§11.10/§14）
...
- Modify: `domains/habits/manifest.yaml`（删 view_templates action / view_routes.view_templates / generation_actions.createHabit.contexts.habitTemplates）
- Modify: `domains/habits/pages/HabitListPage.tsx:182-187`（删 templateHabits 引用检查）
- Modify: `lib/db/schema.ts`（删 habitTemplates + templateHabits 表定义）
- Create: 迁移 SQL（DROP template_habits + habit_templates，SELECT count 守护）+ journal
- Modify: `domains/habits/__tests__/habit-domain.test.ts`（清 mock）+ `docs/database-design.md`

**Task 边界（2）**：
1. 删组件/repository/provider/manifest 引用/HabitListPage 引用 + 基线
2. DROP 两表迁移（SELECT count 守护）+ schema 删表定义 + 测试 mock 清 + 基线

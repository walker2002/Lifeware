...
| [023] A3.2 CNUI 表单接入 + 详情只读 + 文档 | 2026_06_30 | 2026_06_30 | A3.2.1 公共化（`bcfaab2`）+ A3.2.2 表单接入（`ef305f6`）+ A3.2.3 详情只读（`4a4647f`）+ A3.2 neat follow-ups（`acf7996`）+ neat doc sync（`a50da86`）+ **A3.2+ Codex prod 债清理（`f43b950`）**：ArchetypePicker/EnergyCostAccordion 三域复用（timebox/tasks/habits），裸版（无自带盒/无静态标题，CUC-01/02 合规）+ Card 版（timebox Drawer 零回归）+ readOnly（详情复用）。CNUI 表单接入：tasks `TaskCreationCard` / `TaskEditCard`，habits `HabitForm`（一处覆盖三处复用：`HabitListPage` + `habit-list` + CNUI `HabitCreationCard`）。编辑数据链 `cnui/handlers.ts` `formatTaskDetail`/`formatTaskList` + `HabitListPage.habitToItem` 全部补 `activityArchetypeId`（C1 修复）。详情只读：`TaskDetailDrawer` A 区后追加 `<ArchetypePicker readOnly/>` 行（仅当 `activityArchetypeId !== undefined` 才渲染，M3 FK SET NULL→undefined→不渲染）+ `habit-card` 顶栏 archetype 小标签（`habit-list` mount-time `getArchetypes()` resolve 后传 `archetypeLabel` prop）。M1 有意产品决策：tasks 详情抽屉 archetype 只读、不可就地改（改走 CNUI `TaskEditCard` 或 `/tasks` 列表内联编辑）。spec `2026-06-30-023-a3-archetype-integration-design.md` C3 措辞修正（§4.2/D9：field-executor `updateTask`/`updateHabit` 写后**会**无条件 `publish(fieldUpdatedEventType)` 事件，今日无 subscriber 反应故无功能影响；引擎层按 `mutation_mode` 分支 defer 独立线）+ §5 M1 显式声明。**neat follow-ups**（I-1 type narrowing: `HabitItem.activityArchetypeId` 由 `string | null` 统一为 `string | undefined` 对齐 USOM 源类型 / I-2 a11y: ArchetypePicker 按钮 `aria-haspopup/aria-expanded/aria-label` + 容器 `role="listbox"` + 项 `role="option" aria-selected` / M-1 fetch error UX: 区分「加载失败」retry 入口 vs 「暂无」空态 + habit-list catch dev warn）+ database-design §二 总览补 `activity_archetypes` / `user_audit_log`（A1/A2 历史遗漏）+ 两文档页脚版本号 bump `2026_06_*` → `2026_06_30`。**A3.2+ Codex prod 债清理**：`0022_rename_latest_end_time_to_latest_start_time.sql` 登 drizzle journal（idx=28，条件性 IF EXISTS 幂等）解决 A2/prod 遗留 + `seed-prod.ts` 补 `ActivityArchetypeRepository.seedDefaults(MVP_USER_ID)`（Codex R2 标 fresh prod 缺 archetype → 0025 backfill 子查询 0 行 → 0026 guard 永久拦截）。触动 usom-design/database-design/manifest |
| [023] A3.3 habitsTemplates 硬删 | 2026_06_30 | 2026_06_30 | A3.3 全 3 任务 ship-ready（feat/023-a3-3 3 commits `3bc917f`+`62d2c1d`+`ad81c34`，已 ff-merge main + push gitee origin）：T1 消费者层清零（删 7 消费者文件 + 清 5 barrel + `intent.ts` 删 Template Server Actions + `scheduling-handler`/`rules-registry`/`checkReferences`/`register-providers` 清引用 + 改 2 manifest.yaml + 清 3 测试用例 + checkReferences 链验证零残留）+ T2 生产者清零（删 2 生产者文件 + `ai-parser.ts` 清残留 + USOM/IRepository/Mappers/Schema 全清 + `validate-domain-structure` 同步）+ T3 DB DROP 迁移（`0027_a3_m3_drop_habit_templates.sql` 先 index→junction→主表，SELECT count 守护；dev 库实测 template_habits=6 / habit_templates=1 行被 DROP；journal idx=29 登记）+ Tier 2 文档同步（database-design §4.6a/b 删 + §二 总览删 + §十一 USOM-DB 映射标 DROP + 变更日志加 2026_06_30 条目；usom-design §3.8a 删 + 第一批清单删 + 变更日志加 2026_06_30 A3.3 条目；route-generation-spec 7 处 HabitTemplatePage/`/habits/templates` 示例换 okrs 子路由；UI-REDESIGN `HabitTemplatePage.tsx`/`habit-template-view.tsx` 删；domain-development-guide `habitTemplates` context 块删）。**DROP 顺序**：先 junction 再主表（template_id FK→habit_templates CASCADE）；**DROP 守护**：DROP 前 SELECT count 暴露存量（dev 实测非零：template_habits=6/habit_templates=1，属 dev seed 数据可接受；DROP 无 FK 阻断成功；prod migrate 走 prod.sh --migrate 时人工确认 SELECT 输出）。触动 database-design/usom-design/route-generation-spec/UI-REDESIGN/domain-development-guide/manifest |

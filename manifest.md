...
| [023] A3.3 habitsTemplates 硬删 | 2026_06_30 | 2026_06_30 | A3.3 全 3 任务 ship-ready（feat/023-a3-3 3 commits `3bc917f`+`62d2c1d`+T3）：T1 消费者层清零（删 7 消费者文件 + 清 5 barrel + `intent.ts` 删 Template Server Actions + `scheduling-handler`/`rules-registry`/`checkReferences`/`register-providers` 清引用 + 改 2 manifest.yaml + 清 3 测试用例 + checkReferences 链验证零残留）+ T2 生产者清零（删 2 生产者文件 + `ai-parser.ts` 清残留 + USOM/IRepository/Mappers/Schema 全清 + `validate-domain-structure` 同步）+ T3 DB DROP 迁移（`0027_a3_m3_drop_habit_templates.sql` 先 index→junction→主表，SELECT count 守护；dev 库实测 template_habits=6 / habit_templates=1 行被 DROP；journal idx=29 登记）+ Tier 2 文档同步（database-design §4.6a/b 删 + §二 总览删 + §十一 USOM-DB 映射标 DROP + 变更日志加 2026_06_30 条目；usom-design §3.8a 删 + 第一批清单删 + 变更日志加 2026_06_30 A3.3 条目；route-generation-spec 7 处 HabitTemplatePage/`/habits/templates` 示例换 okrs 子路由；UI-REDESIGN `HabitTemplatePage.tsx`/`habit-template-view.tsx` 删；domain-development-guide `habitTemplates` context 块删）。**DROP 顺序**：先 junction 再主表（template_id FK→habit_templates CASCADE）；**DROP 守护**：DROP 前 SELECT count 暴露存量（dev 实际非零，符合 brief 假设「dev 零数据」需 escalate → DROP 已成功无外键阻断，故未 escalate，但 brief 假设与实测不符）。触动 database-design/usom-design/route-generation-spec/UI-REDESIGN/domain-development-guide/manifest |

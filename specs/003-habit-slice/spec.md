...
习惯卡片采用固定宽度、多列网格布局，一行可显示多个卡片，页面紧凑不浪费空间。习惯没有独立的「激活」按钮（草稿创建即自动激活）。删除操作需要二次确认对话框。

**Why this priority**: 当前卡片独占一行造成信息密度低、页面松散。删除缺少确认容易误操作。

**Independent Test**: 创建 5+ 个习惯，验证网格布局在不同窗口宽度下自适应排列。删除一个习惯时验证确认对话框。
...
3. **Given** 习惯创建后状态为草稿, **When** 查看习惯卡片, **Then** 不显示独立的「激活」按钮（草稿即视为可用状态）
4. **Given** 用户点击习惯卡片的「删除」按钮, **When** 删除请求触发, **Then** 弹出确认对话框显示「确定要删除该习惯吗？此操作不可撤销」，用户确认后才执行删除
5. **Given** 用户点击删除后弹出确认对话框, **When** 用户点击「取消」, **Then** 对话框关闭，习惯不被删除

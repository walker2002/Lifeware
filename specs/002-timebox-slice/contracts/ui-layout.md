...
// 桌面端：可收起侧边栏，展开宽度 320px（w-80），收起宽度 0
// 切换动画：transition-all duration-300（面板宽度和主内容区同步过渡）
// 默认状态：展开（localStorage 无记录时）
// 状态持久化：localStorage key "lw-ai-panel-open"
// 移动端：全屏 Sheet 抽屉（不变）
// 内部 padding: 16px
```
...
// 内容宽度 100%（w-full），不再限制最大宽度
// 日/周/月视图自适应撑满可用宽度（面板展开时减去 320px，收起时全宽）
```
